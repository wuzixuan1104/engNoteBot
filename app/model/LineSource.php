...
class LineSource extends Model {
  ...
  public function updateRichmenu() {
    $richmenu = \M\LineRichmenu::last(['where' => ['enable = ?', \M\LineRichmenu::ENABLE_YES]]);
    
    if ($this->lineRichmenuId == $richmenu->id) 
      return false;

    if ($richmenu->richMenuId && !Generator::create4user($this->sid, $richmenu->richMenuId))
      \Log::error('重新建立 richmenu 失敗:' . $this->sid);

    $this->lineRichmenuId = $richmenu->id;
  ...
  public function bindToken($nonce) {
    if (!$nonce)
      return false;
